...
st.set_page_config(
    page_title="Process Prescription",
    page_icon="📋",
    layout="centered",
...
st.markdown('<h1 class="page-header" style="font-family: \'Georgia\', serif; text-align: center; font-size: 3.8rem; color: #52b788; margin-top: 0rem; margin-bottom: 0rem;">MediScan</h1>', unsafe_allow_html=True)
                                        ...
                                        try:
                                            ...
                                            st.markdown(f"""
                                                <div style="padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px; margin: 10px 0;">
                                                    <h4 style="color: #2A5DB0;">Alternative Option: {sub_med_info['name']}</h4>
                                            ...
                                            if "YES" in analysis.upper():
                                                ...
                                                with col1:
                                                    new_qty = st.number_input(
                                                        f"Select quantity for {sub_name}",
                                                        min_value=0,
                                                        max_value=sub_qty,
                                                        value=current_qty,
                                                        key=qty_key
...
with col2:
                                                    if pack_size > 1:
                                                        st.info(f"Pack: {pack_size} units")
            ...
            if submitted:
                ...
                if cart_updated:
                    ...
                    # Force a rerun to update the cart display at the top
                    st.rerun()
                else:
    ...
    """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                invoice_date = st.date_input("Date", datetime.datetime.now())
            ...
            if patient_name.strip():
                try:
                    ...
                    pdf.cell(0, 10, f'Age: {age} years | Gender: {gender}', 0, 1)
                    pdf.cell(0, 10, f'Date: {invoice_date.strftime("%Y-%m-%d")}', 0, 1)
                    pdf.ln(5)
                    ...
                    filename = f"{patient_name}_{age}_{gender}.pdf"
